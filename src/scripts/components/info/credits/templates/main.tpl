<ul class="table-view">
  <li>
    <p>
      <strong>We are very grateful for all the people that helped to
        create this app:</strong>
    </p>
    <p>
    <ul style="list-style-type: none;">
      <li>Dr Richard Pywell <i>(CEH)</i></li>
      <li>Dr David Roy <i>(CEH)</i></li>
      <li>Karolis Kazlauskis <i>(CEH)</i></li>
      <li>Emily Swan <i>(Natural England)</i></li>
      <li>Cath Shellswell <i>(Plantlife)</i></li>
      <li>Mike Green <i>(Natural England)</i></li>
      <li>Dr Markus Wagner <i>(CEH)</i></li>
      <li>Tom Humphrey <i>(BSBI)</i></li>
      <li>Dr Colin Harrower <i>(CEH)</i></li>
      <li>John Redhead <i>(CEH)</i></li>
      <li>Andrew Evans <i>(evansbleep.co.uk)</i></li>
    </ul>
    </p>
    <p>
      This app was part-funded by the Centre for Ecology & Hydrology/Joint Nature Conservation Committee partnership supporting BRC.
    </p>
  </li>
  <li>
    <strong>Photographs:</strong>
    <p>
      Andrea Moro,
      Cath Shellswell,
      Chris Gibson,
      Christian Fischer,
      Emily Swan,
      Forest & Kim Starr,
      Fornax,
      Julia Kruse,
      K.J. Walker,
      Ian Denholm/BSBI,
      Lliam Rooney,
      Malcolm Storey, bioimages.org.uk,
      Willem van Kruijsbergen,
      Rutger Barendse,
      Jan van der Straaten,
      Hagen Graebner,
      Marion Lang,
      Harald Albrecht,
      Markus Wagner,
      Matt Lavin,
      Pete Stroh,
      Rob Hille,
      Stefan Lefnaer</p>
  </li>
  <li>
    <p>The guide has been produced in collaboration with:</p>
    <p>
      <a href="http://www.ceh.ac.uk" target="_blank">Centre for Ecology and Hydrology</a><br/>
      <a href="http://www.bsbi.org.uk/" target="_blank">Botanical Society of Britain and Ireland</a><br/>
      <a href="http://www.plantlife.org.uk/" target="_blank">Plantlife</a><br/>
      <a href="http://www.brc.ac.uk" target="_blank">Biological Records Centre</a><br/>
      <a href="https://www.gov.uk/government/organisations/natural-england" target="_blank">Natural England</a><br/>
    </p>
  </li>
  <li>
    <div class="sponsor-logo-2"></div>
  </li>
</ul>